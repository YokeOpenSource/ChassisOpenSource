...
#define USART6_BUF_SIZE         14
#define MAX_RING_BUF_SIZE       2500
...
unsigned char USART6_TxBuf[ USART6_BUF_SIZE ]  = {0};
...
void USART6_Init(void)
{
    ...
    GPIO_InitTypeDef GPIO_Cfg = { GPIO_Pin_6, GPIO_Mode_AF, GPIO_Speed_100MHz, GPIO_OType_PP, GPIO_PuPd_UP };
    GPIO_Init(GPIOC, &GPIO_Cfg);            // TX

    GPIO_Cfg.GPIO_Pin   = GPIO_Pin_7;
    GPIO_Cfg.GPIO_PuPd  = GPIO_PuPd_NOPULL;
    GPIO_Init(GPIOC, &GPIO_Cfg);            // RX
    /**  @} */
...
void USART6_DMA_Rx_Init(void)
{
    ...
    DMA_DoubleBufferModeConfig(DMA1_Stream1, (uint32_t) USART6_RxBuf1, DMA_Memory_0);
...
unsigned int USART6_Read(unsigned char *data, unsigned short len)
{
    ...
    if(data != NULL)
        result = CircBuf_Read(&USART6_RxCBuf, data, len);
...
{
     if(CircBuf_IsFull(&USART6_RxCBuf))
     {
        return true;
     }
